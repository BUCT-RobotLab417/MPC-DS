...
{        
	...
	nh.param("time_res" ,time_resolution, 0.01);  // 定义时间精度
	nh.param("p_init_x", p_init_x, 0.0);
	nh.param("p_init_y", p_init_y, 0.0);
	nh.param("p_init_z", p_init_z, 0.0);
	
	velocity_cmdsub  = nh.subscribe("command", 1, rcvVelCmdCallBack );  // 发布速度控制指令
	odom_pub  = nh.advertise<nav_msgs::Odometry>("odometry", 1);  // 发布odom

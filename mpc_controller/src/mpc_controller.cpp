...
void MPC_SOLVE::init_solver(std::string _Sm, double& _Ts, int& _Ns, 
                            ...
                            double _gamma, double _safe_dist, double _lambda)
{
    Smetric_type= _Sm;
    Ts_s        = _Ts;
    N_s         = _Ns;
...
bool MPC_SOLVE::imp_solve(Eigen::VectorXd* param1, Eigen::MatrixXd* param2, 
    ...
    if (Smetric_type == "DS") {
        ...
        for(int i=0; i<obs_num; i++){
            ...
            if(!exceed_obs) {
                total_constraints += N_s;
            }
        }
        if(total_constraints > 0) {
            delta_vars = prob.variable(total_constraints);
            prob.subject_to(prob.bounded(0, delta_vars, delta_max_));
        }
    }
    ...
    if (Smetric_type == "DS" && delta_vars.size1() > 0) {
        casadi::MX slack_penalty = lambda_ * casadi::MX::sumsqr(delta_vars);
        cost = cost + slack_penalty;
    }
    ...
    prob.subject_to(prob.bounded(0, v, v_max));
    ...
    for(int i=0; i<obs_num; i++){
        ...
        if(!exceed_obs && choose_num<2){
            ...
            for(int j=0; j<N_s; j++)
            {
                ...
                else if (Smetric_type=="DS")
                {
                    casadi::MX X_   = X_k(casadi::Slice(), j);
                    casadi::MX X_1  = X_k(casadi::Slice(), j+1);
                    casadi::MX hk   = h1(X_,    obs_matrix_s->col(i*N_s+j));
                    casadi::MX hk1  = h1(X_1,   obs_matrix_s->col(i*N_s+j));
                    if (delta_vars.size1() > delta_idx) {
                        casadi::MX delta_k = delta_vars(delta_idx);
                        casadi::MX ds_constraint = hk1 - (1 - gamma_ - delta_k) * hk;
                        prob.subject_to(ds_constraint >= 0);
                        delta_idx++;
                    } else {
                        casadi::MX cbf  = -hk1 + (1-gamma_)*hk;
                        prob.subject_to(cbf <= 0);
    ...
    try{
        ...
        if (Smetric_type == "DS" && delta_vars.size1() > 0) {
            casadi::native_DM delta_values = solution_->value(delta_vars);
            double max_delta = 0.0;
            for(int i = 0; i < delta_values.size1(); i++) {
                max_delta = std::max(max_delta, static_cast<double>(delta_values(i, 0)));
            }
            if(max_delta > 0.01) {
                std::cout << "\033[33m DS activated with max δ = " << max_delta << "\033[0m" << std::endl;
            }
        }
    }
...
void MPC_PLANNER::init(ros::NodeHandle& nh)
{
    nh_ = nh;
    ...
    nh_.param("/mpc_planner/controller_type", controller, 4);
    Smetric_type = controller_ls[controller];
    nh_.param("/mpc_planner/mpc_frequency",   replan_period_, 5.0);
    nh_.param("/mpc_planner/step_time",       Ts_, 0.1);
    nh_.param("/mpc_planner/pre_step",        N_, 25);
    double v_max, v_min, o_max;
    nh_.param("/mpc_planner/v_max",  v_max, 1.5);
    nh_.param("/mpc_planner/v_min",  v_min, 0.8);
    nh_.param("/mpc_planner/o_max",  o_max, 0.8);

    std::cout<< "Smetric_type: "<< Smetric_type<< std::endl;
    ...
    if (Smetric_type == "DS") {
        ...
        nh_.param("/mpc_planner/ds_lambda", lambda, 50.0);
        nh_.param("/mpc_planner/ds_gamma", gamma, 0.25);
        
        std::cout << "\033[32m DS Method Initialized - Lambda: " << lambda 
                  << ", Gamma: " << gamma << "\033[0m" << std::endl;
    } else {
        Q = {1.0, 1.0, 0.05}; 
        R = {0.1, 0.05};    
        gamma = 0.3;
    ...
    solver.init_solver(Smetric_type, Ts_, N_, v_max, v_min, o_max, Q, R, gamma, safe_dist, lambda);
...
{
    double dist = (cur_xy - end_xy).norm();
    if(dist < 0.1){
...
void MPC_PLANNER::choose_goal_state()
{
    ...
    for(int i = 0; i < N_; i++){
        int index;
        if (num + i > waypoint_num - 1) {
            index = waypoint_num - 1;
        } else {
            index = num + i;
        }
...
void MPC_PLANNER::pub_Predict_traj(std::vector<double>& _predict_traj){
    ...
    predict_traj.header.frame_id    = "odom";
...
void MPC_PLANNER::rcvOdomCallBack(nav_msgs::OdometryPtr msg)
{
    ...
    cur_xy(0) = msg->pose.pose.position.x;
    cur_xy(1) = msg->pose.pose.position.y;
    has_odom_ = true;
}
...
void MPC_PLANNER::rcvTrajCallBack(nav_msgs::PathPtr msg)
{
    ...
    if (waypoint_num > 0) {
        end_xy(0) = msg->poses[waypoint_num - 1].pose.position.x;
        end_xy(1) = msg->poses[waypoint_num - 1].pose.position.y;
    }
    has_traj_ = true;
}

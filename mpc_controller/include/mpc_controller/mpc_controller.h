...
private:
    ...
    casadi::MX h1(casadi::MX& _curpos, Eigen::VectorXd _obs); // control barrier function

    double lambda_;             // 松弛变量惩罚系数
    double delta_max_;          // 松弛变量最大值
...
public:
    ...
    void init_solver(std::string _Sm, double& _Ts, int& _Ns, 
                    ...
                    double _gamma, double _safe_dist, double _lambda = 100.0);
...
private:
    ...
    int controller;                             // 控制器类型
    double replan_period_;                      // 重规划周期 = 1 / 频率
    double Ts_;                                 // 离散时间
    int N_;                                     // time horizon


    Eigen::VectorXd cur_state_;                 // 机器人当前状态
    Eigen::Vector2d cur_xy, end_xy;
    Eigen::VectorXd end_state_;                 // 机器人目标状态
    Eigen::MatrixXd global_path_;
    Eigen::MatrixXd goal_state_;                // mpc中用到的state,每一列代表一个状态[x,y,theta].T
